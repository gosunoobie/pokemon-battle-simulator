...
All seven sound/animation batches are accepted and active in move preview, solo simulation and private multiplayer: **44 unique moves and 45 sound assets**, including Psychic’s separate impact accent. Each host uses the latest approved animation, native sound regions, gain and visual pace. Batch 6’s Ancient Power supersedes Batch 4’s version; Batch 5 retains the final Eruption/Blizzard corrections and reviewed edit tapers. Batch 7’s songs, hearts, healing finishes and circling ducks run through their full endings.

Accepted batch sounds support Web Audio browsers and device sample rates without a Chrome-version restriction. Each animation allows a short, cancellable preload window for a newly encountered sound before its clock starts; late files never play out of sync. See the [playback fix and validation](docs/SFX_ACCEPTED_ROLLOUT.md).

Super-effective and not-very-effective hits now play their matching complete recordings at the effectiveness reveal in both battle simulators, including Substitute hits. Move preview adds the super-effective cue to its existing fixed examples. These sounds share the Sound / Move sounds controls, retain natural tails, and stop on skip, reset, mute or stale playback. Details are in [effectiveness audio](docs/SFX_EFFECTIVENESS_AUDIO.md).

Solo and private battles also play a Poké Ball activation sound at opening and a faint sound when the outgoing Pokémon starts fainting. These share the adjusted battle-effects volume; Pokémon cries remain separate. Reconnects and effects-off snapshots do not replay transitions. Verify all four event assets with `npm run sfx:events:check`; see [transition audio](docs/SFX_TRANSITION_AUDIO.md).

The local `/sfx-bench?batch=sync-007` page and all six earlier batch pages now replay accepted finals. Historical comparisons and browser feedback remain archived. See the [rollout record](docs/SFX_ACCEPTED_ROLLOUT.md); verify source pins and delivered sound bytes with `npm run sfx:accepted:check`.
