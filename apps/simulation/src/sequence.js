...
export function introOverlay(view, run, playerLabel = 'Your team') {
  return {
    ...
    opponentName: run?.opponent?.name ?? 'Opponent', opponentTitle: run?.opponent?.title ?? 'Battle trainer',
...
export function createBattleSequence({ presenter, onOverlay, timers = globalThis }) {
  ...
  async function present(batch, { effectsEnabled = true, reducedMotion = false } = {}) {
    ...
    const { before, after, run = null, playerLabel } = batch
    ...
    try {
      if (introduce && effectsEnabled) {
        ...
        publish({ ...introOverlay(after, run, playerLabel), animated: true, motion: reducedMotion ? 'reduced' : 'full', durationMs })
