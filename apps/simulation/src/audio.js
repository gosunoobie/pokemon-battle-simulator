...
export function createSimulationAudio({ draftSfxEnabled = import.meta.env?.VITE_SIMULATION_DRAFT_SFX_ENABLED !== 'false', ...options } = {}) {
  return createBattleAudio({ categoryVolumes: BATTLE_AUDIO_MIX, impactVolume: BATTLE_IMPACT_VOLUME, ...options, technicalSoundPack: draftSfxEnabled ? technicalSoundPack : null })
}
