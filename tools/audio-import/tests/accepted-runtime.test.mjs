...
test('second acceptance preserves all six exact native plans and the independent Psychic impact', async () => {
  ...
  assert.deepEqual(result.catalog.provenance.batches.map(row => row.batchId), ['sync-001', 'sync-002', 'sync-003', 'sync-004', 'sync-005', 'sync-006', 'sync-007'])
...
test('third acceptance preserves the six measured native plans and approved visual accent identity', async () => {
  ...
  assert.equal(Object.keys(combined.catalog.moves).length, 44)
  assert.equal(Object.keys(combined.catalog.assets).length, 45)
