...
export default function bubble(context) {
...
function bubble(tl, move) {
    const follow = charge(tl, 62, 0.25, 1.02)
    tl.to(attacker, { x: home.x - 7, duration: 0.16, onUpdate: follow }, 0)
      .to(attacker, { x: home.x + 5, duration: 0.14, onUpdate: follow }, 0.16)
      .to(attacker, { x: home.x, duration: 0.3, onUpdate: follow }, 1.5)
    const source = mouth(5)
    for (let i = 0; i < 9; i++) {
      const lane = [0, -1, 1][i % 3]
      flyingBubble(tl, {
        source: { x: source.x, y: source.y + lane * 8 }, target: { x: (focus.x), y: (focus.y) + lane * 20 },
        radius: 12 + (i % 3) * 5, at: 0.34 + i * 0.085, duration: 0.68 + (i % 3) * 0.035,
        arc: lane * 36, sway: 7, phase: i * 0.4, ease: 'power1.inOut',
      })
    }
    tl.to(impactGlow, { alpha: 0.18, duration: 0.12 }, 1.02)
      .to(impactGlow, { alpha: 0, duration: 0.35 }, 1.35)
    addHit(tl, move, 1.02, { recoil: 7, shake: 1, duration: 0.55 })
    tl.call(() => {}, [], 2.45)
  }
  bubble(tl,move)
}
