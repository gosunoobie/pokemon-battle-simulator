...
export default function sing(context) {
  ...
  function update(time){
    ...
    notes.forEach((g,i)=>{const age=time-.2-i*.12,u=clamp(age/.76),p={x:from.x+(to.x-from.x)*u,y:from.y+(to.y-from.y)*u+Math.sin(Math.PI*u)*Math.sin(u*5-time*3+i)*bow}
      fit(g,p,15);g.scale.x*=Math.sign(temporary.scale.x);g.rotation=Math.sin(time*4+i)*.13*Math.sign(temporary.scale.x);g.alpha=age>=0&&age<1.02?Math.min(1,age*12)*Math.min(1,(1.02-age)*5):0})
  ...
  onFrame(update)
  tl.to(lull,{alpha:1,duration:.25},.88).to(lull,{alpha:0,duration:.38},1.94)
