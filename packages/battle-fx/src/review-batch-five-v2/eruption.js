...
// The nine sound crests increase a continuously flowing lava volley.
...
export default function eruption(context) {
  ...
  ERUPTION_VOLLEY_ARRIVALS.forEach((arrive, index) => {
    ...
    const size = .66 + index % 4 * .11
    glow(lava, 58 * size, 74 * size, 0xff4818, .85); glow(lava, 31 * size, 39 * size, 0xffba39); glow(lava, 12 * size, 16 * size, 0xfff0af)
    lavas.push({ lava, lane, launch, arrive, flight, source: null, impact: null, height: 84 + index % 5 * 19 })
    for (let j = 0; j < 6; j++) {
  ...
  lavas.forEach(item => tl.call(() => { item.source = fit(socket('vent', true), 42) }, [], item.launch))
  function update(time) {
    ...
    for (const item of lavas) {
      ...
      const target = item.impact ?? fit({ x: live.x + item.lane * 13, y: live.y + item.lane * 7 }, 42)
