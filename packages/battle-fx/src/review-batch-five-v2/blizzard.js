...
// Second review of Blizzard: incoming small ice lodges and grows before each established late shatter.
...
function blizzard(tl, move) {
  ...
  const crystals = pulses.map((impact, i) => {
    ...
    return { crystal, flash, shards, impact, start: impact - .42, lodge: impact - .16, origin: null, collision: null }
  })
  ...
  onFrame(time => {
    ...
    crystals.forEach((entry, i) => {
      const { crystal, flash, shards, start, lodge, impact } = entry
      crystal.clear(); flash.clear(); crystal.alpha = 0; flash.alpha = 0
      if (time >= start && !entry.origin) entry.origin = pointInField(socket('emission', true), 8)
      const live = targetSocket('center', true), lane = (i % 3 - 1) * Math.min(13, context.target.metrics.height / unit * .075)
      const destination = pointInField({ x: live.x, y: live.y + lane }, 8)
      if (time >= start && time <= impact) {
        ...
        const rotation = Math.atan2(destination.y - origin.y, destination.x - origin.x)
        // Its front tip lodges at the destination; the same small crystal then
        // grows backward from that fixed attachment before shattering.
        const growthTime = clamp((time - lodge) / .13, 0, 1), growth = growthTime * growthTime * (3 - 2 * growthTime)
        const finalLength = i === 0 ? 64 : 78 + (i % 3) * 8, finalWidth = i === 0 ? 17 : 23
        const length = 20 + (finalLength - 20) * growth, width = 5.5 + (finalWidth - 5.5) * growth
        const polygon = [0, 0, -length * .54, -width, -length, 0, -length * .54, width]
        const size = fitPolygon(tip, rotation, polygon)
        crystal.position.copyFrom(tip); crystal.rotation = rotation; crystal.scale.set(size)
        ...
        crystal.alpha = Math.min(1, (time - start) / .035)
      }
