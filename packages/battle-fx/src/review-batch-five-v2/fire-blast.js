...
// Second local review: an early larger orb opens into five independent streams
// of expanding flame puffs, with the continuous flow law inspired by Flamethrower.
// The original and first review recipes remain independent and unchanged.
...
export default function fireBlast(context) {
  ...
  const contact = timing.contact, launchAt = 1.5425, chargeAt = .1
  ...
  function update(time) {
    ...
    fit(mouthGlow, mouth, 95); mouthGlow.alpha = time < .06 ? 0 : .9 * clamp((time - .06) / .24) * (1 - clamp((time - 1.6525) / .25))
