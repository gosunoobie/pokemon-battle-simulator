...
// Second review of Thunder: retain the approved timing and replace only the late blast artwork.
...
function thunder(tl, move) {
  ...
  onFrame(time => {
    ...
    const radiusX = Math.max(1, Math.min(170, ground.x - bounds.left - 4, bounds.right - ground.x - 4))
    const radiusY = Math.max(1, Math.min(180, radiusX * 1.05, ground.y - bounds.top - 4))
    ...
    for (let i = 0; i < 13; i++) {
      const size = (.98 - i * .054) * swell
      dome(radiusX * size, radiusY * size, i < 6 ? 0xffb62c : 0xffe990, intensity * (.011 + i * .0015))
    }
    dome(radiusX * .19 * swell, radiusY * .24 * swell, 0xfff5c0, intensity * .32)
    dome(radiusX * .07 * swell, radiusY * .09 * swell, 0xfffff0, intensity * .65)
    for (let i = 0; i < boltLengths.length; i++) {
      ...
      const root = { x: Math.sin(i * 2.3) * radiusX * .047, y: -4 }
      const shimmer = Math.sin(time * 32 + i * 1.7), flicker = .7 + .3 * Math.sin(time * 26 + i * 2.1) ** 2
      const at = (p, bend) => ({
        x: clamp(root.x + Math.cos(angle) * radiusX * length * p + Math.sin(angle) * radiusX * bend, -radiusX + 3, radiusX - 3),
        y: Math.min(-4, root.y + Math.sin(angle) * radiusY * length * p - Math.cos(angle) * radiusY * bend),
      })
      const points = [root, at(.27, .035), at(.46, -.026), at(.68, .025 + shimmer * .006), at(.94, -.006)]
      for (const [width, color, alpha] of [[5.8, 0xffb923, .14], [2.7, 0xffde69, .72], [1.05, 0xffffeb, .95]]) {
        ...
        if (i % 2 === 0) {
          ...
          discharge.moveTo(points[2].x, points[2].y).lineTo(branch.x, Math.min(-4, branch.y))
