...
// Review-only extension of the original Blizzard. The registered recipe remains unchanged.
...
function blizzard(tl, move) {
  ...
  const crystals = pulses.map((impact, i) => {
    ...
    return { crystal, flash, shards, impact, start: impact - (i === 0 ? .3 : .23), origin: null, collision: null }
  })
  ...
  onFrame(time => {
    ...
    crystals.forEach((entry, i) => {
      const { crystal, flash, shards, start, impact } = entry
      ...
      if (time >= start && time <= impact) {
        const p = clamp((time - start) / (impact - start), 0, 1), origin = entry.origin
        ...
        const length = i === 0 ? 64 : 78 + (i % 3) * 8, width = i === 0 ? 17 : 23
        ...
        crystal.poly(polygon).fill({ color: i % 2 ? 0xb4eaff : 0x99d6ee, alpha: .48 })
          .stroke({ width: 1.7, color: 0xf3fdff, alpha: .94 })
