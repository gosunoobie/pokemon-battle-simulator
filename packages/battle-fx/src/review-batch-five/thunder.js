...
// Review-only extension of the original Thunder. The registered recipe remains unchanged.
...
function thunder(tl, move) {
  ...
  // The second electrical burst is cosmetic: the original bolt remains the only result cue.
  const second = new Container(); second.label = 'thunder-second-impact'; temporary.addChild(second)
  const discharge = new Graphics(); discharge.label = 'thunder-second-discharge'; second.addChild(discharge)
  ...
  onFrame(time => {
    ...
    const live = targetSocket('center', true)
    const center = { x: clamp(live.x, bounds.left + 4, bounds.right - 4), y: clamp(live.y, bounds.top + 4, bounds.bottom - 4) }
    second.position.copyFrom(center)
    const room = Math.min(center.x - bounds.left, bounds.right - center.x, center.y - bounds.top, bounds.bottom - center.y)
    const radius = Math.max(1, Math.min(142, room - 3))
    const age = time - onset, rising = clamp(age / (crest - onset), 0, 1)
    const fade = time <= crackleEnd ? 1 : clamp((fadeEnd - time) / (fadeEnd - crackleEnd), 0, 1)
    const core = (time < crest ? .25 + .75 * rising : .3 + .7 * Math.exp(-(time - crest) * 12)) * fade
    const ringRadius = radius * (.2 + .73 * Math.min(1, age / .19))
    discharge.circle(0, 0, radius * (.1 + .19 * rising)).fill({ color: 0xffd72f, alpha: core * .38 })
      .circle(0, 0, radius * (.06 + .075 * rising)).fill({ color: 0xffffef, alpha: core * .94 })
      .circle(0, 0, ringRadius).stroke({ width: 2.2, color: 0xffed7b, alpha: Math.exp(-age * 6) * .75 * fade })
    for (let i = 0; i < 13; i++) {
      const angle = i * Math.PI * 2 / 13 + .035 * Math.sin(time * 23 + i)
      const flicker = .62 + .38 * Math.sin(time * 43 + i * 2.7) ** 2
      const reach = radius * (.5 + .42 * flicker) * (.35 + .65 * rising)
      const points = [
        [Math.cos(angle) * reach * .13, Math.sin(angle) * reach * .13],
        [Math.cos(angle + .19) * reach * .43, Math.sin(angle + .19) * reach * .43],
        [Math.cos(angle - .08) * reach * .59, Math.sin(angle - .08) * reach * .59],
        [Math.cos(angle + .035) * reach, Math.sin(angle + .035) * reach],
      ]
      for (const [width, color, alpha] of [[4.2, 0xffc52d, .38], [1.55, 0xffffeb, .96]]) {
        discharge.moveTo(...points[0]); points.slice(1).forEach(p => discharge.lineTo(...p))
        discharge.stroke({ width, color, alpha: alpha * flicker * fade, cap: 'round', join: 'round' })
        discharge.moveTo(...points[1]).lineTo(Math.cos(angle - .26) * reach * .72, Math.sin(angle - .26) * reach * .72)
          .stroke({ width: width * .65, color, alpha: alpha * flicker * fade, cap: 'round' })
      }
    }
    for (let i = 0; i < 38; i++) {
      const birth = onset + i / 37 * (crackleEnd - onset - .045), age = time - birth, life = .28
      if (age < 0 || age > life) continue
      const phase = age / life, angle = i * 2.3999632297, distance = radius * phase * (.55 + (i % 4) * .1)
      const x = clamp(center.x + Math.cos(angle) * distance, bounds.left + 2, bounds.right - 2) - center.x
      // Positive y remains downward in either horizontally reflected effect space.
      const y = clamp(center.y + Math.sin(angle) * distance * .6 + radius * .35 * phase * phase, bounds.top + 2, bounds.bottom - 2) - center.y
      flecks.circle(x, y, 1.25 + (i % 3) * .3).fill({ color: i % 2 ? 0xffffff : 0xffdf5a, alpha: (1 - phase) * fade })
    }
  })
