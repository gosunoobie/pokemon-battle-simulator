...
export const timing = Object.freeze({ contact: .88, duration: 3.44, markers: Object.freeze([
  { id: 'last-whistle-pulse', label: 'Final continuing whistle pulse', timeSeconds: 2.68 },
  { id: 'seed-drift-fade', label: 'Final moving seed drift fade', timeSeconds: 2.99 },
  { id: 'music-fade', label: 'Final leaf and pulse fade', timeSeconds: 3.14 },
]) })
...
export default function grassWhistle(context) {
  ...
  const ripples=Array.from({length:17},(_,i)=>{const g=new Graphics();g.label=`grass-whistle-pulse-${i}`;g.alpha=0;temporary.addChild(g);return g})
  ...
  function update(time){
    ...
    ripples.forEach((g,i)=>{const age=time-.28-i*.15,u=clamp(age/.6),p={x:from.x+(to.x-from.x)*u,y:from.y+(to.y-from.y)*u-Math.sin(Math.PI*u)*Math.min(17,room(from)/3,room(to)/3)}
      fit(g,p,24);g.rotation=Math.atan2(to.y-from.y,to.x-from.x);g.clear().ellipse(0,0,5+u*4,10+u*9).stroke({color:i%2?0xe4f2b6:0xa3d7a3,width:1.7,alpha:.85});g.alpha=age>=0&&age<.85?Math.min(1,age*12)*Math.min(1,(.85-age)*6)*clamp((timing.duration-time)/.3):0})
    fit(drift,to,r*1.4)
  ...
  onFrame(update)
  tl.to(leaf,{alpha:1,duration:.16},.06).to(leaf,{alpha:0,duration:.3},3.14)
    .to(drift,{alpha:1,duration:.3},.8).to(drift,{alpha:0,duration:.45},2.99)
