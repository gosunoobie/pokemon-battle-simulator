...
export default function leafBlade(context) {
  ...
  function outline(g) {
    // Retain only the upper crescent; the former center vein is its new lower edge.
    return g.moveTo(0, 0).bezierCurveTo(-length * .09, -length * .22, -length * .48, -length * .48, tip.x, tip.y)
      .bezierCurveTo(-length * .6, -length * .3, -length * .24, -length * .06, 0, 0).closePath()
  }
  ...
  const veinY = x => {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const curveX = -length * (3 * v * v * u * .24 + 3 * v * u * u * .6 + u * u * u)
      if (curveX > x) low = u; else high = u
    }
    const u = (low + high) / 2, v = 1 - u
    return -length * (3 * v * v * u * .06 + 3 * v * u * u * .3 + u * u * u * .14)
  }
  for (let i = 1; i <= 4; i++) {
    const u = i / 6, a = { x: -length * u, y: -length * .15 * Math.sin(u * Math.PI) }
    const b = { x: -length * (u + .065), y: -length * (.14 + .1 * Math.sin(u * Math.PI)) }
    let start = 0
    if (a.y > veinY(a.x)) {
      // Keep the surviving portion of each original vein, without lines in the cut-away half.
      let low = 0, high = 1
      for (let j = 0; j < 28; j++) {
        const t = (low + high) / 2, x = lerp(a.x, b.x, t), y = lerp(a.y, b.y, t)
        if (y > veinY(x)) low = t; else high = t
      }
      start = high
    }
    leaf.moveTo(lerp(a.x, b.x, start), lerp(a.y, b.y, start)).lineTo(b.x, b.y)
