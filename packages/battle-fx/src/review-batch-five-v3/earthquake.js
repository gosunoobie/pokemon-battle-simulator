...
// Third review restores the original level ellipses; only the fault follows ground depth.
...
export default function earthquake(context) {
  ...
  births.forEach((birth, i) => {
    const ring = new Graphics(); ring.label = `earthquake-v3-wave-${i}`; temporary.addChild(ring)
    rings.push({ ring, birth, life: i < 4 ? .9 : Math.min(.72, 2.22 - birth), i })
  })
  const cracks = new Graphics(); cracks.label = 'earthquake-v3-fault'; temporary.addChild(cracks)
  ...
  for (let wave = 0; wave < 3; wave++) for (let i = 0; i < 10; i++) {
    ...
    g.label = `earthquake-v3-pebble-${wave}-${i}`; temporary.addChild(g)
  ...
  for (let wave = 0; wave < 6; wave++) for (let i = 0; i < 8; i++) {
    ...
    g.label = `earthquake-v3-dust-${wave}-${i}`; temporary.addChild(g)
  ...
  function update(time) {
    for (const { ring, birth, life, i } of rings) {
      ...
      // Keep the ring horizontal, as in the restored original Earthquake.
      // Ground depth belongs to the separate fault route, never a shear of the ring.
      for (let j = 0; j <= 64; j++) {
        const a = j * Math.PI * 2 / 64, dx = Math.cos(a) * rx
        const q = point(center.x + dx, center.y + Math.sin(a) * ry, 3)
