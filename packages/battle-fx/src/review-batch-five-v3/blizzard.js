...
// Third review of Blizzard: seven body attachments grow into upright blue-white crystal blades.
  ...
  onFrame(time => {
    ...
    crystals.forEach((entry, i) => {
      ...
      if (time >= start && time <= impact) {
        ...
        // grows from that fixed attachment and turns upright before shattering.
        const growthTime = clamp((time - lodge) / .13, 0, 1), growth = growthTime * growthTime * (3 - 2 * growthTime)
        const rotation = flightRotation + (Math.PI / 2 - flightRotation) * growth
