...
test('loading never replays a missed cue, and playback uses decoded duration with gain headroom', async t => {
  ...
  assert.equal(calls.gains[0].gain.value * calls.gains[1].gain.value * calls.gains[4].gain.value, 0.21)
  player.setVolume(1)
  assert.equal(calls.gains[0].gain.value * calls.gains[1].gain.value * calls.gains[4].gain.value, 0.35)
  player.setVolume(3)
...
test('two voices are bounded; replacement and natural completion disconnect only owned nodes', async t => {
  ...
  assert.equal(calls.gains[4].disconnected, 1)
