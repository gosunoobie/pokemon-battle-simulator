...
import oldThunder, { timing as oldThunderTiming } from '../packages/battle-fx/src/review-batch-five-v2/thunder.js'
import oldBlizzard, { timing as oldBlizzardTiming } from '../packages/battle-fx/src/review-batch-five-v2/blizzard.js'
...
function harness(sourceId = 'source', custom = false, builders = definitions) {
  ...
  const fx = createBattleFx({ glowTexture: Texture.WHITE, assetLoader: async () => Texture.WHITE,
    effects: Object.fromEntries(Object.entries(builders).map(([id, definition]) => [id, { ...MOVE_EFFECTS[id], ...definition }])),
...
test('third weather review preserves every v2 cue, pulse and duration without replacing either earlier builder', () => {
...
test('Thunder larger bright dome has varied thick branches and remains rooted above the visible feet', async () => {
...
test('each small Blizzard crystal lodges and grows upright continuously before the unchanged shatter', async () => {
  ...
  for (const sourceId of ['source', 'target']) for (const [i, impact] of pulses.entries()) {
    ...
    try {
      ...
      const bigBounds = crystal.getBounds()
      assert.ok(bigBounds.height > bigBounds.width * 1.3, 'final crystal is a tall vertical diamond in either perspective')
      assert.ok(Math.abs(crystal.rotation - Math.PI / 2) < .0001)
...
test('Thunder third review increases glow coverage and separates thick primary bolts from fine secondary bolts', async () => {
  for (const sourceId of ['source', 'target']) {
    const versions = [
      { thunder: { build: oldThunder, ...oldThunderTiming }, blizzard: { build: oldBlizzard, ...oldBlizzardTiming } },
      definitions,
    ]
    const samples = []
    for (const builders of versions) {
      const h = harness(sourceId, false, builders)
      try {
        const run = h.play('thunder'); await tick(); h.tl.time(1.5825, false)
        const discharge = h.node('thunder-second-discharge'), b = discharge.getBounds()
        const fills = discharge.context.instructions.filter(item => item.action === 'fill')
        const cores = discharge.context.instructions.filter(item => item.action === 'stroke'
          && [0xffffeb, 0xffffe8].includes(item.data.style.color)
          && item.data.path.instructions.filter(part => part.action === 'lineTo').length >= 4)
        samples.push({ width: b.width, height: b.height, alpha: fills.reduce((sum, item) => sum + item.data.style.alpha, 0),
          peakAlpha: Math.max(...fills.map(item => item.data.style.alpha)), cores: cores.map(item => item.data.style.width) })
        run.cancel(); assert.equal((await run.finished).status, 'cancelled'); clean(h)
      } finally { h.dispose() }
    }
    const [previous, revised] = samples
    assert.ok(revised.width > previous.width * 1.1, `larger impact covers more horizontal space ${sourceId}: ${JSON.stringify(samples)}`)
    assert.ok(revised.height > previous.height * 1.1, 'larger upper dome covers more vertical space')
    assert.ok(revised.alpha > previous.alpha * 1.5, 'soft layered glow carries substantially more light')
    assert.ok(revised.peakAlpha > previous.peakAlpha, 'white-yellow center is brighter')
    assert.ok(Math.max(...revised.cores) > Math.max(...previous.cores) * 3, 'primary bolts have thicker ivory cores')
    assert.ok(Math.max(...revised.cores) / Math.min(...revised.cores) >= 3, 'thick primary and fine secondary bolts remain visibly distinct')
    assert.ok(new Set(revised.cores).size >= 5)
  }
})

test('Blizzard uses seven distinct attachments spread across the visible body before vertical growth', async () => {
  const pulses = [1.5165, 1.929, 2.004, 2.079, 2.154, 2.229, 2.304]
  for (const sourceId of ['source', 'target']) for (const custom of [false, true]) {
    const h = harness(sourceId, custom), contacts = []
    try {
      const run = h.play('blizzard'); await tick()
      for (const [index, impact] of pulses.entries()) {
        h.tl.time(impact, false)
        const point = h.point(h.node(`blizzard-late-impact-${index}`)), center = h.target.anchor('visualCenter')
        const dx = point.x - center.x, dy = point.y - center.y
        assert.ok(Math.abs(dx) <= h.target.metrics.width * .4 && Math.abs(dy) <= h.target.metrics.height * .4, 'attachment remains inside the visible body')
        assert.ok(Math.abs(h.node(`blizzard-large-crystal-${index}`).rotation - Math.PI / 2) < .0001)
        contacts.push(point)
      }
      for (let i = 0; i < contacts.length; i++) for (let j = i + 1; j < contacts.length; j++) {
        assert.ok(Math.hypot(contacts[i].x - contacts[j].x, contacts[i].y - contacts[j].y) > 5, 'crystals cannot reuse a previous attachment')
      }
      assert.ok(Math.max(...contacts.map(p => p.x)) - Math.min(...contacts.map(p => p.x)) > h.target.metrics.width * .45)
      assert.ok(Math.max(...contacts.map(p => p.y)) - Math.min(...contacts.map(p => p.y)) > h.target.metrics.height * .5)
      h.tl.time(blizzardTiming.duration, false); assert.equal((await run.finished).status, 'completed'); clean(h)
    } finally { h.dispose() }
  }
})
